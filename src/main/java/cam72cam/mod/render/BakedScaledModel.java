...
/**
 * Internal class to scale an existing Baked Model, do not use directly!
 */
class BakedScaledModel implements IBakedModel {
    ...
    public BakedScaledModel(IBakedModel source, Matrix4 transform, Plane plane) {
        ...
        List<BakedQuad> all = new ArrayList<>(source.getQuads(null, null, rand));
        for (EnumFacing dir : EnumFacing.values()) {
        ...
        for (BakedQuad quad : all) {
            ...
            quadCache.get(quad.getFace()).add(quad);
        }
    }
    ...
    public List<BakedQuad> getQuads(IBlockState state, EnumFacing side, long rand) {
        ...
        } else if (quadCache.get(side) == null) {
